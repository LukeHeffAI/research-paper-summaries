...
- **Music & SFX.** A small library of assets (`intro` theme, `outro`, transition `sting`, optional low `bed`) under `$DATA_DIR/assets/audio/`, referenced by `cue`. SFX are optional and sparse (an interview rarely needs them). Theme/bed assets are placeholder/curated for now; we will likely **generate the theme via ElevenLabs** (music/SFX) later — a generation path stays available behind the audio port for both the theme and the rare SFX cue.
