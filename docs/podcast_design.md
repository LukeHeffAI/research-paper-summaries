...
- **Target length** ~8 minutes by default (config-tunable, `podcast.target_minutes`) — the prompt targets a turn/words budget mapping to roughly that runtime; a guide, not a hard cap.
...
- **Music & SFX.** A small library of assets (`intro` theme, `outro`, transition `sting`, optional low `bed`), referenced by `cue`. **Shipped placeholders live in the committed repo `assets/audio/`** (the `ASSETS_DIR` default; small generated `.wav` stings); a user can override `ASSETS_DIR` to a `$DATA_DIR/assets/audio/` path to point at their own curated theme. SFX are optional and sparse (an interview rarely needs them). Theme/bed assets are placeholder/curated for now; we will likely **generate the theme via ElevenLabs** (music/SFX) later — a generation path stays available behind the audio port for both the theme and the rare SFX cue (tracked in `FUTURE_FIXES.md`). A missing or unmapped asset is logged and skipped so the episode still renders.
