...
* **Native PDF** -- a base64 ``document`` content block placed *before* the text
  ...
  models). Inline base64 ONLY here -- the caller (``SummariseStage``) guards the
  raw-byte size and falls back to extracted text + section-split for PDFs too big
  to inline safely. Native-large-PDF via the Files API (``client.beta.messages.*``
  + ``files-api-2025-04-14``) is deferred (see FUTURE_FIXES.md).
...
* **Thinking/effort** -- adaptive thinking + ``output_config.effort``; never
  ``budget_tokens`` / ``temperature`` (400 on current models). For straightforward
  summarisation on Sonnet, thinking is omitted (cheaper). On Opus we force
  ``thinking={"type": "adaptive"}`` so reasoning is not leaked into the visible
  output and does not pollute the structured prose fields.
...
# Model-family prefix that needs adaptive thinking forced on to avoid leaking
# reasoning into the visible (prose) output.
...
class AnthropicLLMClient:
    ...
    def complete_structured(
        ...
        *,
        ...
        content = self._build_user_content(document, instruction)
    ...
    def _build_user_content(self, document: LLMDocument, instruction: str) -> list[dict[str, Any]]:
        """Build the user-turn content blocks: document first, then instruction.

        Native PDF is inlined as a base64 ``document`` block (the only native path
        here). The caller guards the raw-byte size before reaching this point, so an
        oversized PDF never arrives as a native document.
        """
        ...
        return [inline_block, {"type": "text", "text": instruction}]

    def _thinking_param(self) -> dict[str, Any] | None:
        """Adaptive thinking on Opus (avoids reasoning leaking into prose); off on Sonnet."""
        if self._model.startswith(_OPUS_PREFIX):
